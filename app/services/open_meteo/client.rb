...
module OpenMeteo
  class Client
    ...
    def get(path, params: {}, action:)
      uri = build_uri(path, params)
      response = request(uri)

      unless response.is_a?(Net::HTTPSuccess)
        log_failure(action:, uri:, status: response.code)
        raise ResponseError.new("Open-Meteo #{action} failed with HTTP #{response.code}", action:, status: response.code)
      end

      JSON.parse(response.body)
    rescue JSON::ParserError => error
      log_failure(action:, uri:, error:)
      raise ParseError.new("Open-Meteo #{action} returned invalid JSON", action:)
    rescue Timeout::Error, SocketError, SystemCallError, Net::OpenTimeout, Net::ReadTimeout => error
      log_failure(action:, uri:, error:)
      raise RequestError.new("Open-Meteo #{action} request failed", action:)
    end
    ...
    def log_failure(action:, uri:, status: nil, error: nil)
      ...
      logger.warn("Open-Meteo request failed #{details.join(' ')}")
    end
  end
end
